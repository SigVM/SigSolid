...
contract A {
    ...
    function emitfunc(bytes32 DataSent) public {
		...
		assembly {
			emitsig(this_priceFeedUpdate_key,ONE_HOUR,this_priceFeedUpdate_dataslot,32)
	    }
...
contract B {
	...
	function bindfunc() public view {
		...
		assembly {
			bindsig(dut_address,dut_priceFeedUpdate_key,sload(priceReceive_key_slot))
	    }
    ...
    function detachfunc() public view {
		...
		address dut_address = address(dut);
		assembly{
			detachsig(dut_address,dut_priceFeedUpdate_key,sload(priceReceive_key_slot))
		}
