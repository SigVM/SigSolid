...
public:
	...
	/// Tries to dynamically load an evmc vm supporting evm1 or ewasm.
	/// @returns vmc::VM(nullptr) on failure.
	static evmc::VM& getVM(std::string const& _path = {});

	/// Tries to load all defined shared libraries.
	/// @param _vmPaths paths to multiple evmc shared libraries.
	/// @throw Exception if multiple evm1 or multiple ewasm evmc vms where defined.
	/// @returns true, if an evmc vm was defined supporting evm1,
	///          false, otherwise, or if a defined evmc vm was not able to load.
