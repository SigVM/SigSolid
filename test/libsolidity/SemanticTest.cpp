	...
	m_enforceViaYul(enforceViaYul)
{
	...
	if (runWithEwasm && m_ewasmHost == nullptr)
		runWithEwasm = false;
...
TestCase::TestResult SemanticTest::run(ostream& _stream, string const& _linePrefix, bool _formatted)
{
	...
	for (bool compileViaYul: set<bool>{!m_runWithoutYul, m_runWithYul || m_enforceViaYul})
		...
		{
			bool skip = run(results, _stream, _linePrefix, _formatted, compileViaYul, compileToEwasm);
			if (skip)
				continue;
		}
...
bool SemanticTest::run(TestCase::TestResult& _results, ostream& _stream, const string& _linePrefix, bool _formatted, bool _compileViaYul, bool _compileToEwasm)
{
	try
	{
		TestCase::TestResult result = run(_compileViaYul, _compileToEwasm, _stream, _linePrefix, _formatted);
